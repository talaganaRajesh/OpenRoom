...
export default function MessageItem({ message, isCurrentUser, onReply }) {
  ...
  // Highlight code when Prism is loaded
  useEffect(() => {
  ...
  const detectCode = (text) => {
    const codePatterns = [
      ...
      /`[^`\n]+`/g, // Inline code (if substantial)
      /<[^>]+>[\s\S]*?<\/[^>]+>/g, // HTML tags with content
      /function\s+\w+\s*\([^)]*\)\s*\{/g, // JavaScript functions
      /class\s+\w+[\s\S]*?\{/g, // Class declarations
      /import\s+.*from\s+['"][^'"]+['"]/g, // Import statements
      /export\s+(default\s+)?(function|class|const|let|var)/g, // Export statements
      /#include\s*<.*>/g, // C/C++ includes
      /public\s+static\s+void\s+main/g, // Java main method
      /def\s+\w+\s*\([^)]*\)\s*:/g, // Python functions
      /\w+\s*:\s*\w+\s*[{;][\s\S]*?}/g, // CSS-like syntax
      /\$\w+\s*=.*[;]?/g, // PHP variables
      /SELECT\s+.*FROM\s+/gi, // SQL queries
      /console\.log\(/g, // Console statements
      /document\./g, // DOM manipulation
      /<!DOCTYPE\s+html>/gi, // HTML DOCTYPE
    ];
    
    // Check if text has multiple lines with programming syntax
    const lines = text.split('\n');
    if (lines.length > 2) {
      const programmingLines = lines.filter(line => 
        /^[\s]*[{}\[\];][\s]*$/.test(line) || // Brackets/semicolons on their own lines
        /^[\s]*(if|for|while|function|class|def|public|private|protected|import|export|const|let|var)[\s\(]/.test(line) ||
        /^[\s]*[<>\/]/.test(line) || // HTML-like tags
        /[\{\};\(\)].*[\{\};\(\)]/.test(line) // Multiple programming punctuation
      ).length;
  ...
  const detectLanguage = (text) => {
    ...
    if (text.includes('public static void main') || text.includes('System.out') || text.includes('import java.')) return 'java';
    if (text.includes('def ') && text.includes(':') || text.includes('import ') && !text.includes('from ')) return 'python';
    if (text.includes('console.log') || text.includes('document.') || text.match(/function\s+\w+/)) return 'javascript';
    if (text.includes('interface ') && text.includes(': ') || text.includes('type ')) return 'typescript';
    if (text.includes('<!DOCTYPE') || text.match(/<html[\s>]/i) || text.match(/<\/\w+>/)) return 'markup';
    if (text.includes('{') && text.includes(':') && text.includes(';') && text.match(/[\w-]+\s*:\s*[^;]+;/)) return 'css';
    if (text.includes('<?php') || text.includes('$_')) return 'php';
    if (text.match(/SELECT\s+.*FROM/gi) || text.match(/INSERT\s+INTO/gi)) return 'sql';
    if (text.includes('using System') || text.includes('namespace ')) return 'csharp';
    if (text.includes('"scripts"') && text.includes('"dependencies"')) return 'json';
    if (text.startsWith('$') || text.includes('Get-') || text.includes('Set-')) return 'powershell';
    if (text.includes('#!/bin/bash') || text.includes('echo ') || text.includes('grep ')) return 'bash';
  ...
  const handleCopy = async (e) => {
    ...
    try {
      ...
      textArea.value = isCodeMessage ? formatCodeText(message.text) : message.text;
  ...
  return (
    <div
      ...
      <div className={`flex ${isCodeMessage ? 'w-4/5 max-w-4xl' : 'md:max-w-2xl min-w-24 max-w-72 w-full'} flex-col relative`}>
        ...
        >
          {isCodeMessage ? (
            // Code container with syntax highlighting and custom width
            <div className="relative w-full">
              ...
              {/* Code content with controlled dimensions */}
              <div className="bg-zinc-900 dark:bg-zinc-950 w-full h-64 overflow-auto">
                <pre className="!m-0 !p-4 !bg-transparent text-sm leading-6 h-full" style={{ fontFamily: 'Monaco, Menlo, "Ubuntu Mono", Consolas, monospace' }}>
                  <code
                    ref={codeRef}
        ...
        <div className={`absolute ${isCurrentUser ? "-left-20" : "-right-20"} top-1/2 -translate-y-1/2 flex gap-1`}>
          {/* Copy button (only show for non-code messages since code has its own copy) */}
          {!isCodeMessage && (
            <button
              onClick={handleCopy}
              className="text-xs p-1 bg-gray-100 hover:bg-gray-200 dark:bg-zinc-700 dark:hover:bg-zinc-600 
              rounded-full opacity-0 group-hover:opacity-70 hover:opacity-100 transition-opacity group/tooltip-copy"
          ...
          <button
            ...
            className="text-xs p-1 bg-gray-100 hover:bg-gray-200 dark:bg-zinc-700 dark:hover:bg-zinc-600 
            rounded-full opacity-0 group-hover:opacity-70 hover:opacity-100 transition-opacity group/tooltip-reply"
            aria-label="Reply"
          >
